pipeline {
    ...
    environment {
        DEPLOY_DIR = '/opt/services/byz-ingest'
        BINARY_NAME = 'byz-ingest'
        SUPERVISOR_PROGRAM = 'byz-ingest'
    }
        ...
        stage('Deploy') {
            steps {
                sh '''
                    ...
                    curl -sf "http://127.0.0.1:8100/healthz" || true
                '''
            }
        }
    }
    ...
    post {
        success {
            echo 'byz-ingest deployed'
        }
        failure {
            echo 'Build or deploy failed. Ensure Go, Solr, Kafka, and sudoers allow deploy.'
        }
    }
}
